    ...
    const fetchStats = async () => {
      try {
        ...
        const { data: reportsData, error: reportsError } = await supabase
          .from("scamReports")
          .select("id")
          .gte("created_at", new Date(todayStart).toISOString()) // Start of today
          .lt("created_at", new Date(tomorrowStart).toISOString()); // Start of tomorrow
        ...
        const { data: searchesData, error: searchedError } = await supabase
          .from("edge_logs")
          .select("*, metadata") // Explicitly select the 'metadata' field
          .filter("metadata.request.method", "eq", "GET");
        ...
        setStats({
          ...
          totalSearchesToday: searchesData.length, // You will need to implement logic for this if needed
        });
      } catch (error) {
