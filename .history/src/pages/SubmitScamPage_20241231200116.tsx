...
const SubmitScamPage = () => {
  ...
  const handleUploads = async () => {
    ...
    setUploadStatuses(Array(images.length).fill("Uploading..."));
    ...
    const promises = images.map(async (image, index) => {
      try {
        ...
        setUploadStatuses((prevStatuses) => {
          ...
          newStatuses[index] = "Upload successful!";
          return newStatuses;
        });
  ...
  return (
    <div>
      ...
      <br />
      <button onClick={handleUploads}>Upload All</button>
      <div style={{ marginTop: "10px" }}>
        {uploadStatuses.map((status, index) => (
          <p key={index}>{`Image ${index + 1}: ${status}`}</p>
        ))}
      </div>
    </div>
  );
