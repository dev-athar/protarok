    ...
    try {
      const { error: insertError } = await supabase
        ...
        .insert([
          {
            ...report,
            approvedBy: supabase.auth.user()?.id, // Ensure admin is authenticated
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      valueGetter: (params) => {
        ...
        if (date) {
          return new Date(date); // Ensure the date is converted to a valid Date object
        }
        return null; // If date is invalid or missing, return null
      },
      valueFormatter: (params) => {
        const date = params.value as Date;
        return date ? date.toLocaleString() : "Invalid Date"; // Display "Invalid Date" if the date is invalid
      },
    },
    {
      field: "actions",
