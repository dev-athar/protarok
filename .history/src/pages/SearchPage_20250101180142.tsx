    ...
    const fetchSearchResults = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from("scamReports")
          .select("scamProfileName", "bankDetails", "profileUrl")
          .or(
            `scamProfileName.ilike.%${searchTerm}%,
             bankDetails.ilike.%${searchTerm}%,
             profileUrl.ilike.%${searchTerm}%`
          );
        ...
        <div className="flex flex-col gap-4 mt-2">
          {loading ? (
            ...
            searchResults.map((result, index) => (
              <div
                key={index}
                className="border rounded p-4 bg-white shadow-md hover:shadow-lg cursor-pointer"
                onClick={() => handleResultClick(result.id)} // Assuming `id` is a unique identifier in your scamReports table
              >
